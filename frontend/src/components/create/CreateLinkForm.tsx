...
import { Link as LinkIcon, Zap, ChevronDown, AlertCircle, Sparkles, Lock } from "lucide-react";
    ...
    <div className="w-full max-w-3xl mx-auto">
      ...
      <div className="flex flex-col items-center text-center pt-6 pb-8">
        ...
        <p className="text-sm sm:text-base text-text-muted mt-3 max-w-lg font-sans">
          Create clean, memorable links in seconds and track visitor engagement with production-grade
          telemetry.
        </p>
      </div>
      ...
      <div className="bg-surface-card rounded-2xl shadow-md border border-border-subtle p-6 sm:p-8 transition-all">
        {/* Auth gate — shown while loading or unauthenticated */}
        {!isLoading && !isAuthenticated ? (
          <div className="flex flex-col items-center text-center py-6 space-y-4">
            <div className="w-14 h-14 rounded-2xl bg-blue-50 border border-blue-200/60 flex items-center justify-center">
              <Lock className="w-7 h-7 text-primary-container" />
            </div>
            <div>
              <h2 className="text-base font-bold text-text-charcoal">
                Sign in to create short links
              </h2>
              <p className="text-xs text-text-muted mt-1 max-w-xs leading-relaxed">
                A free account lets you shorten URLs, track clicks, set custom aliases, and manage
                link expiration — all in one place.
              </p>
            </div>
            <div className="flex items-center gap-3 pt-1">
              <Link
                href="/signup"
                className="px-5 py-2.5 bg-primary-container hover:bg-blue-700 text-white text-sm font-semibold rounded-xl shadow-sm transition-all active:scale-[0.98]"
              >
                Create Free Account
              </Link>
              <Link
                href="/login"
                className="px-5 py-2.5 bg-surface-card border border-border-subtle hover:border-slate-400 hover:bg-slate-50 text-text-charcoal text-sm font-semibold rounded-xl transition-all active:scale-[0.98] shadow-2xs"
              >
                Sign In
              </Link>
            </div>
